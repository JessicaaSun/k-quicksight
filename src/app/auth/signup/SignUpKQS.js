...
export default function SignUpKQS() {
  ...
  return (
    <Outlet>
      <div className="min-h-screen max-sm:w-full sm:w-full 2xl:pe-40 md:pe-0 sm:pe-0  max-sm:pe-0 flex items-center max-sm:justify-center sm:justify-center md:justify-end  bg-background-color">
        <div className="min-h-screen fixed top-0 rounded-r-3xl left-0 z-10 md:flex sm:hidden max-sm:hidden justify-end items-center w-[30%] bg-[#5686EF]">
          <button
            onClick={() => router.push("/")}
        ...
        <Image
          ...
          width={200}
          height={200}
          className="w-[40%] md:flex sm:hidden max-sm:hidden absolute bottom-0 left-[9rem] z-20 h-auto"
        />
        <SignUpForm />
      </div>
    </Outlet>
