...
import {Button, Modal, ModalBody, ModalContent, ModalHeader, useDisclosure, Card, CardHeader, CardFooter} from "@nextui-org/react";
...
import { useAllAnalysisFileQuery, useDeleteAnalysisFileMutation } from "@/store/features/analysis/Analysis";
...
const Page = () => {
  ...
  const {data:allAnalysis} = useAllAnalysisFileQuery({userId: user?.data.id, page: 1, size: 100, title: ''})
                      ...
                      allAnalysis?.results.map((item, index) => (
                          ...
                          <Card className="w-full bg-black hover:scale-105 transition-all h-[300px] col-span-12 sm:col-span-5">
                              ...
                              <CardFooter className="absolute bg-white bottom-0 border-t-1 z-10 justify-between">
                                <div>
                                  <p className="text-black text-tiny">{getTrimIntoColumnOnlyDate(item.created_at)}</p>
                                  <p className="text-black text-tiny">{item.file.file}</p>
                                </div>
                                <DeleteAnalysisFile uuid={item.uuid} filename={item.title} />
                              </CardFooter>
                            </Card>
