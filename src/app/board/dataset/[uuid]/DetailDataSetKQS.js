...
const DetailDatasetKQS = ({ uuid }) => {
  ...
  const {
    ...
    isLoading,
  } = useGetFileDetailQuery({ uuid: uuid, size: 0, page: 1 });
  const {
    data: fileOverview,
  ...
  return (
    <div className={"p-5"}>
      <p
        className={
          "text-3xl font-medium text-primary-color dark:text-third-color"
        }
      >
        Detail
      </p>
      <div className="flex justify-between">
        <div className={"flex gap-5 mt-3"}>
          <p
            className={
              "text-lg font-medium text-text-color dark:text-third-color"
            }
          >
            {fileDetail?.file}
          </p>
      ...
      <div className={"flex flex-col gap-3"}>
        <p
          className={
            "text-primary-color font-semibold text-medium dark:text-third-color"
          }
        >
          Total records: {fileDetail?.total}{" "}
        </p>
...
export default DetailDatasetKQS;
