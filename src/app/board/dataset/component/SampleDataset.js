  ...
  return (
    <>
      <Button
        onPress={onOpen}
        className={
          "text-white w-1/2 bg-primary-color border-2 border-white shadow-lg"
        }
      >
        Sample dataset
      </Button>
      <Modal size="3xl" isOpen={isOpen} onOpenChange={onOpenChange}>
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1 text-2xl text-text-color">
                Pick our sample dataset.
              </ModalHeader>
              <ModalBody>
                <div className={"flex justify-between items-center w-full"}>
                  <p className={"text-2xl text-primary-color font-medium"}>
                    All Dataset
                  </p>
                  {/*<SearchDataset />*/}
                </div>
