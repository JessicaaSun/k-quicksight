...
export default function GetStart_boxs() {
    ...
    return (
        <div className='lg:px-[10%] md:px-5 px-3 py-20 flex flex-col gap-40 '>
            {
                data.map((item, index) => (
                    // eslint-disable-next-line react/jsx-key
                    <div
                        className={`lg:flex md:flex justify-center items-center gap-10 ${index === 1 ? 'flex-row-reverse' : ''} hover:scale-105 transition-all`}
                    >
                        <div className='mb-5'>{item.out_number}</div>
                        <div className={` lg:min-w-[980px] relative px-10 py-5 gap-10 bg-primary-color lg:min-h-[183px] ${index === 1 ? ' flex-row-reverse text-right' : ''} rounded-3xl lg:flex justify-start items-center`}>
                            <div className={`${index === 1 ? '-left-40 top-7 -scale-x-100 w-[420px]' : index === 0 ? '-right-32 -top-40 w-[420px]' : '-right-32 top-20 w-[320px]'} lg:absolute md:absolute lg:block md:block hidden `}>
    ...
    const data = [
        {
            ...
            description: "Make data-driven decisions effortlessly, without technical expertise."
        },
        {
            title: "Data Enthusiasts",
            description: "Dive deeper into data with machine learning capabilities to uncover meaningful insights."
        },
        {
            title: "Researchers and Academics",
            description: "Explore data with precision and efficiency for research and academic purposes."
        },
    ]
    return (
        <div className='lg:flex md:flex w-full block gap-20 justify-between items-center'>
            <div className='flex flex-col gap-16 lg:w-1/2 md:w-1/2 w-full'>
                {
                    data.map((item, index) => (
                        // eslint-disable-next-line react/jsx-key
                        <div className={`lg:min-w-[649.856px] relative p-5 rounded-xl ${index === 1 && "text-right"} md:min-w-[449.856px] min-w-full bg-background-color border-2 border-primary-color`}>
                            <h3 className={`text-primary-color absolute ${index === 1 && "-top-5 right-5"} -top-5 bg-background-color px-5`}>{item.title}</h3>
                            <p className={`text-descriptive-color font-normal`}>{item.description}</p>
                        </div>
                    ))
                }
            </div>
            <Image src={userbase} alt='userbase' className='lg:w-1/3 md:md-1/2 w-full -scale-x-100' />
        </div>
    )
}
...
export function FeedBackCard() {
    const [swapper, setSwapper] = useState([
        // eslint-disable-next-line react/jsx-key
        <div className="bg-thirt-color transition-all hover:scale-105 -translate-x-1/2 -translate-y-1/4 border-3 rotate-6 absolute z-10 -top-14 border-background-color lg:w-[609px] md:w-[500px] h-[290px] rounded-xl">
            <p className="text-xl">The predictive analytics feature has given us a competitive edge in the market. K-QuickSight is truly ahead of the curve.</p>
            <div className="flex gap-5 justify-start items-center">
                <Image src={jessi_profile} alt="jessi_profile.jpg" className="max-w-[78px] max-h-[78px] rounded-full" />
        ...
        // eslint-disable-next-line react/jsx-key
        <div className="bg-primary-color transition-all hover:scale-105 -translate-x-1/2 -translate-y-1/4  rotate-3 border-3 absolute z-10 -top-7 border-background-color lg:w-[609px] md:w-[500px] h-[290px] rounded-xl">
            <p className="text-xl">The predictive analytics feature has given us a competitive edge in the market. K-QuickSight is truly ahead of the curve.</p>
            <div className="flex gap-5 justify-start items-center">
                <Image src={jessi_profile} alt="jessi_profile.jpg" className="max-w-[78px] max-h-[78px] rounded-full" />
                <div className="flex flex-col gap-1">
        ...
        // eslint-disable-next-line react/jsx-key
        <div className="absolute hover:scale-105 transition-all top-0 -translate-x-1/2 -translate-y-1/4 z-10 lg:w-[609px] md:w-[500px] h-[290px] flex flex-col gap-10 rounded-xl bg-background-color p-5 border-3 border-primary-color">
            <p className="text-xl">The predictive analytics feature has given us a competitive edge in the market. K-QuickSight is truly ahead of the curve.</p>
            <div className="flex gap-5 justify-start items-center">
                <Image src={jessi_profile} alt="jessi_profile.jpg" className="max-w-[78px] max-h-[78px] rounded-full" />
                <div className="flex flex-col gap-1">
    ...
    return (
        <div className="relative mt-52">
            {swapper}
            <Button onClick={handleSwap} className='mt-60 bg-thirt-color text-background-color font-semibold'>Next user <svg width="25" height="26" viewBox="0 0 25 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13.846 13.2155L8.77652 8.05929C8.67871 7.9632 8.60069 7.84826 8.54701 7.72117C8.49334 7.59409 8.46509 7.4574 8.46391 7.31909C8.46272 7.18078 8.48864 7.04361 8.54013 6.91559C8.59162 6.78758 8.66767 6.67127 8.76383 6.57347C8.85999 6.47566 8.97433 6.39831 9.10019 6.34594C9.22606 6.29356 9.36091 6.26721 9.4969 6.26841C9.63288 6.26961 9.76727 6.29835 9.89221 6.35294C10.0172 6.40753 10.1302 6.48689 10.2246 6.58638L16.0181 12.4791C16.2101 12.6744 16.318 12.9393 16.318 13.2155C16.318 13.4918 16.2101 13.7567 16.0181 13.952L10.2246 19.8447C10.1302 19.9442 10.0172 20.0236 9.89221 20.0782C9.76727 20.1327 9.63288 20.1615 9.4969 20.1627C9.36091 20.1639 9.22606 20.1375 9.10019 20.0852C8.97433 20.0328 8.85999 19.9554 8.76383 19.8576C8.66767 19.7598 8.59162 19.6435 8.54013 19.5155C8.48864 19.3875 8.46272 19.2503 8.46391 19.112C8.46509 18.9737 8.49334 18.837 8.54701 18.7099C8.60069 18.5828 8.67871 18.4679 8.77652 18.3718L13.846 13.2155Z" fill="white" />
            </svg>
            </Button>
        </div>
    );
            ...
            >
                ...
                <SwiperSlide className='hover:scale-105 transition-all top-0 -translate-x-1/2 -translate-y-1/4 z-10 lg:w-[609px] md:w-[500px] h-[290px] flex flex-col gap-10 rounded-xl bg-primary-color text-background-color p-5 border-3 border-background-color'>
                    ...
                    <div className="flex gap-5 justify-start items-center mt-5">
                        <Image src={jessi_profile} alt="jessi_profile.jpg" className="max-w-[78px] max-h-[78px] rounded-full" />
                        <div className="flex flex-col gap-1">
                            <p className="text-2xl text-background-color font-semibold">Jessica sun</p>
                            <p className="capitalize">Data scientist of google</p>
                        </div>
                    </div>
                </SwiperSlide>

                <SwiperSlide className='hover:scale-105 transition-all top-0 -translate-x-1/2 -translate-y-1/4 z-10 lg:w-[609px] md:w-[500px] h-[290px] flex flex-col gap-10 rounded-xl bg-thirt-color text-background-color p-5 border-3 border-background-color'>
                    <p className="text-xl">The predictive analytics feature has given us a competitive edge in the market. K-QuickSight is truly ahead of the curve.</p>
                    <div className="flex gap-5 justify-start items-center mt-5">
                        <Image src={jessi_profile} alt="jessi_profile.jpg" className="max-w-[78px] max-h-[78px] rounded-full" />
                        <div className="flex flex-col gap-1">
