                ...
                <div className={'flex flex-col gap-3'}>
                    {
                        file?.map((item, index) => (
                            <div key={item.id} className={'hover:bg-primary-color cursor-pointer hover:text-white transition-all px-3 py-3 flex items-center justify-between bg-white rounded-lg border-1 border-gray-200 shadow-sm'}>
                                <div>
                                    <Link href={`/board/dataset/${item.uuid}`} className={'text-lg font-medium hover:underline flex gap-3 justify-center items-center flex-wrap'}>{item.file} <BsDot /> <span className={'text-sm'}>{getTrimIntoColumnOnlyDate(item.created_at || item.createAt)}</span> </Link>
